...
def evaluate(data, model, name):
    ...
    return speed, acc, p, r, f, pred_results,gold_results, gazes
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if end%data.HP_batch_size == 0:
                ...
                loss_adv, tag_seq_adv = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen,
                                                              layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask,
                                                              mask, batch_label, batch_bert, bert_mask)
        ...
        speed, acc, p, r, f, pred_labels,gold_labels, gazs = evaluate(data, model, "dev")
        ...
        if current_score > best_dev:
            if seg:
                ...
                print( "Exceed previous best acc score:", best_dev_p)
        ...
        # ## decode test
        speed, acc, p, r, f, pred_labels,gold_labels, gazs = evaluate(data, model, "test")
...
def load_model_decode(model_dir, data, name, gpu, seg=True):
    ...
    start_time = time.time()
    speed, acc, p, r, f, pred_results,gold_results, gazs = evaluate(data, model, name)
    end_time = time.time()
...
def print_results(pred, modelname="PER"):
    toprint = []
    for sen in pred:
        sen = " ".join(sen) + '\n'
        toprint.append(sen)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelname', default="PER")
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/PER.dset")
    parser.add_argument('--train', default="你的数据路径+/data/PER/train.char.bmes")
    parser.add_argument('--dev', default="你的数据路径+/data/PER/dev.char.bmes" )
    parser.add_argument('--test', default="你的数据路径+/data/PER/test.char.bmes")
    parser.add_argument('--seg', default="True")
    parser.add_argument('--extendalphabet', default="True")
    parser.add_argument('--raw')
    parser.add_argument('--output',default="你的数据路径+/output/output.txt")
    ...
    char_emb = "你的数据路径+/data/gigaword_chn.all.a2b.uni.ite50.vec"
    bichar_emb = "你的数据路径+/data/gigaword_chn.all.a2b.bi.ite50.vec"
    gaz_file = "你的数据路径+/data/ctb.50d.vec"
    ...
    if status == 'train':
        ...
        data.HP_use_count = args.use_count
        data.generate_instance_with_gaz(test_file,'test')
        pred_results, gold_results = load_model_decode(save_model_dir, data, 'test', gpu, seg)
        print(pred_results)
        print(gold_results)
        print("p: ", precision_score(gold_results, pred_results))
        print("r: ", recall_score(gold_results, pred_results))
        print("f1: ", f1_score(gold_results, pred_results))
        # 评估test true:labels  pred:result
        print(classification_report(gold_results, pred_results, mode='strict', scheme=IOBES))
    else:
